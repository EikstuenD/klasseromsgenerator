...
// Markering og flytting
let editMode = false;
let deskCounter = 0;
let selectedDeskIds = new Set(); // Holder styr på hvilke pulter som er valgt

// Dragging variabler
let isDraggingDesks = false;
let isSelecting = false;
let dragStartMouse = { x: 0, y: 0 };
let dragStartDeskPositions = {}; // Husker startposisjon for alle valgte pulter
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Setup events for klasserommet (for markering)
...
/* 1. ELEV HÅNDTERING (Uendret logikk) */
...
function addDesk(top = 100, left = 100) {
    ...
    return deskId; // Returner ID så vi kan velge dem ved preset
}
...
function addPreset(type) {
    ...
    selectedDeskIds.clear(); // Fjern gammel markering
    offsets.forEach(offset => {
        const newId = addDesk(startY + offset.y, startX + offset.x);
        selectedDeskIds.add(newId); // Marker de nye pultene automatisk
    });
...
function toggleEditMode() {
    ...
    selectedDeskIds.clear(); // Fjern valg når vi bytter modus
    renderDesks(); 
}
...
/* 4. DRAG & SELECT LOGIKK (NYTT OG AVANSERT) */

// Når man klikker på en pult i Edit Mode
function handleDeskMouseDown(e, deskId) {
    if (!editMode) return;
    e.stopPropagation(); // Hindre at vi starter selection box
    
    // Hvis man holder SHIFT, toggle valg
    if (e.shiftKey) {
        ...
        // Hvis pulten ikke allerede er valgt, velg KUN denne (vanlig klikk)
        // Men hvis den allerede ER valgt, behold utvalget (for å kunne flytte gruppa)
        if (!selectedDeskIds.has(deskId)) {
    ...
    // Lagre startposisjon for ALLE valgte pulter for jevn flyt
...
// Når man klikker i tomrommet (for å markere)
function handleRoomMouseDown(e) {
    if (!editMode) return;
    // Klikk utenfor pult = fjern valg (med mindre shift)
    if (!e.shiftKey) {
        selectedDeskIds.clear();
        renderDesks();
    }
    ...
    selectionBox = {
        startX: e.clientX - roomRect.left,
        startY: e.clientY - roomRect.top
    };
...
function handleGlobalMouseMove(e) {
    ...
    if (isDraggingDesks) {
        ...
        selectedDeskIds.forEach(id => {
            ...
            if (deskData && startPos) {
                ...
                // Oppdater DOM direkte for ytelse
    ...
    if (isSelecting) {
        ...
        // Sjekk kollisjon med pulter for å markere dem live
        desks.forEach(desk => {
            // Enkel kollisjonssjekk
            if (x < desk.left + 100 && x + w > desk.left &&
                y < desk.top + 60 && y + h > desk.top) {
                selectedDeskIds.add(desk.id);
            } else {
                // Hvis shift ikke holdes, fjern selection hvis utenfor
                if (!e.shiftKey) selectedDeskIds.delete(desk.id);
            }
        });
        
        // Vi må oppdatere visuelt (litt tungt, men ok for 30 pulter)
...
function handleGlobalMouseUp(e) {
    if(isDraggingDesks) {
        isDraggingDesks = false;
        // Posisjoner er allerede oppdatert i desks-arrayet under mousemove
    }
    if(isSelecting) {
        isSelecting = false;
        document.getElementById('selection-box').style.display = 'none';
        renderDesks(); // Clean up visuals
    }
}

/* 5. ELEV BYTTE OG LÅS (Uendret logikk) */
